...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	referralService := service.NewReferralService(client, userRepository, redeemCodeRepository, settingService, billingCacheService, apiKeyAuthCacheInvalidator)
	...
	redeemService := service.NewRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, referralService, affiliateService)
	...
	studioAuthHandler := handler.NewStudioAuthHandler(authService, userService, settingService, totpService)
	...
	imageCreationSessionStore := repository.NewImageCreationSessionStore(redisClient)
	imageCreationSessionService := service.NewImageCreationSessionService(imageCreationSessionStore, userService)
	imageCreationSessionHandler := handler.NewImageCreationSessionHandler(imageCreationSessionService, apiKeyService)
	imageCreationRepository := repository.NewImageCreationRepository(client)
	imageCreationService := service.NewImageCreationService(imageCreationRepository)
	imageCreationHandler := handler.NewImageCreationHandler(imageCreationService)
	...
	pricingHandler := handler.NewPricingHandler(billingService, gatewayService, apiKeyService)
	referralHandler := handler.NewReferralHandler(referralService)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, studioAuthHandler, userHandler, apiKeyHandler, imageCreationSessionHandler, imageCreationHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, pricingHandler, referralHandler, openAIQuotaAutoResetService)
