...
func TestReadRequestBodyWithPrealloc_DecodesXGzip(t *testing.T) {
	var buf bytes.Buffer
	gw := gzip.NewWriter(&buf)
	if _, err := gw.Write([]byte(samplePayload)); err != nil {
		t.Fatalf("gzip write: %v", err)
	...
	req := newRequestWithBody(t, buf.Bytes(), "x-gzip")
	got, err := ReadRequestBodyWithPrealloc(req)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestReadRequestBodyWithPrealloc_RejectsCorruptGzip(t *testing.T) {
...
func TestReadRequestBodyWithPrealloc_RejectsCorruptDeflate(t *testing.T) {
	req := newRequestWithBody(t, []byte("not actually deflate"), "deflate")

	_, err := ReadRequestBodyWithPrealloc(req)
	if err == nil {
		t.Fatal("expected error for corrupt deflate body, got nil")
	}
}

func TestReadRequestBodyWithPrealloc_RejectsOversizedCompressedBody(t *testing.T) {
	oversized := bytes.Repeat([]byte("a"), maxDecompressedBodySize+1)

	var buf bytes.Buffer
	gw := gzip.NewWriter(&buf)
	if _, err := gw.Write(oversized); err != nil {
		t.Fatalf("gzip write: %v", err)
	}
	if err := gw.Close(); err != nil {
		t.Fatalf("gzip close: %v", err)
