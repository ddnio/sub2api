...
func soonestResetTimePtr(t time.Time) *time.Time { return &t }
...
func TestFilterBySoonestReset_PicksSoonestFutureWindow(t *testing.T) {
	...
	accounts := []accountWithLoad{
		accWithWindowEnd(1, soonestResetTimePtr(later)),
		accWithWindowEnd(2, soonestResetTimePtr(soon)),
		accWithWindowEnd(3, soonestResetTimePtr(later)),
	}
...
func TestFilterBySoonestReset_IgnoresNilAndExpiredWindows(t *testing.T) {
	...
	accounts := []accountWithLoad{
		...
		accWithWindowEnd(2, soonestResetTimePtr(expired)), // 已过期，视为无活跃窗口
		accWithWindowEnd(3, soonestResetTimePtr(active)),  // 唯一活跃窗口
	}
	got := filterBySoonestReset(accounts)
	require.Len(t, got, 1)
...
func TestFilterBySoonestReset_NoActiveWindowReturnsAll(t *testing.T) {
	...
	accounts := []accountWithLoad{
		...
		accWithWindowEnd(2, soonestResetTimePtr(expired)),
	}
	got := filterBySoonestReset(accounts)
...
func TestFilterBySoonestReset_TiedSoonestKeepsAll(t *testing.T) {
	...
	accounts := []accountWithLoad{
		accWithWindowEnd(1, soonestResetTimePtr(end)),
		accWithWindowEnd(2, soonestResetTimePtr(end)),
		accWithWindowEnd(3, soonestResetTimePtr(now.Add(5*time.Hour))),
	}
	got := filterBySoonestReset(accounts)
