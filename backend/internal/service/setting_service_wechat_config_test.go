...
func TestSettingService_GetWeChatConnectOAuthConfig_UsesDatabaseOverrides(t *testing.T) {
	...
	require.Equal(t, "wx-db-app", got.LegacyAppID)
	require.Equal(t, "wx-db-secret", got.LegacyAppSecret)
	require.Equal(t, "wx-db-app", got.AppIDForMode("open"))
	require.Equal(t, "wx-db-secret", got.AppSecretForMode("open"))
