...
func isClaudeOpus47Model(modelID string) bool {
	normalized := strings.ToLower(claude.NormalizeModelID(modelID))
...
func normalizeClaudeOAuthRequestBody(body []byte, modelID string, opts claudeOAuthNormalizeOptions) ([]byte, string) {
	...
	if isClaudeOpus47Model(modelID) {
		...
		!gjson.GetBytes(out, "top_k").Exists() &&
		!claude.IsOpus55(modelID) {
