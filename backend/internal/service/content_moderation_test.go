...
func TestContentModerationCheck_PreBlockWithoutAPIKeysBlocks(t *testing.T) {
	...
	cfg.Mode = ContentModerationModePreBlock
	cfg.APIKeys = nil
	cfg.BlockStatus = http.StatusFailedDependency
	cfg.BlockMessage = "内容审计不可用"
	rawCfg, err := json.Marshal(cfg)
	require.NoError(t, err)
	...
	decision, err := svc.Check(context.Background(), ContentModerationCheckInput{
		Protocol: ContentModerationProtocolOpenAIChat,
		Body:     []byte(`{"messages":[{"role":"user","content":"prompt requiring audit"}]}`),
	})
...
func TestContentModerationCheck_PreBlockModerationAPIFailureBlocks(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requestCount++
		http.Error(w, "moderation unavailable", http.StatusBadGateway)
	}))
	defer server.Close()
	...
	require.Equal(t, ContentModerationActionError, repo.logs[0].Action)
	require.False(t, repo.logs[0].Flagged)
	require.Contains(t, repo.logs[0].Error, "moderation api status 502")
}
