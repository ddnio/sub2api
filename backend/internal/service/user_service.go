...
func (s *UserService) GetProfileIdentitySummaries(ctx context.Context, userID int64, user *User) (UserIdentitySummarySet, error) {
	...
	return UserIdentitySummarySet{
		...
		LinuxDo: buildProviderIdentitySummary("linuxdo", records),
		OIDC:    buildProviderIdentitySummary("oidc", records),
		WeChat:  buildProviderIdentitySummary("wechat", records),
	}, nil
}
...
func buildProviderIdentitySummary(provider string, records []UserAuthIdentityRecord) UserIdentitySummary {
	summary := UserIdentitySummary{Provider: provider}
