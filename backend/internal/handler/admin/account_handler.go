...
func (f *BulkUpdateAccountFilters) isEmpty() bool {
	if f == nil {
		return true
	}
	return strings.TrimSpace(f.Platform) == "" &&
		strings.TrimSpace(f.Type) == "" &&
		strings.TrimSpace(f.Status) == "" &&
		strings.TrimSpace(f.Group) == "" &&
		strings.TrimSpace(f.Search) == "" &&
		strings.TrimSpace(f.PrivacyMode) == ""
}
...
func (h *AccountHandler) BulkUpdate(c *gin.Context) {
	...
	if len(req.AccountIDs) == 0 && req.Filters.isEmpty() {
