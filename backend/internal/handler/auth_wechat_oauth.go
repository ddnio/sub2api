...
func (h *AuthHandler) CompleteWeChatOAuthRegistration(c *gin.Context) {
	...
	if err := applyPendingOAuthAdoption(c.Request.Context(), h.entClient(), h.userService, session, decision, &user.ID); err != nil {
...
func (h *AuthHandler) ensureWeChatRuntimeIdentityBinding(
	...
	err = ensurePendingOAuthIdentityForUser(dbent.NewTxContext(ctx, tx), tx.Client(), &dbent.PendingAuthSession{
		ProviderType:           strings.TrimSpace(identity.ProviderType),
