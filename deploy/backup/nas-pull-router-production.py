...
import os,pathlib,subprocess,tempfile,hashlib,json,tarfile,fcntl,datetime,shlex
os.umask(0o077)
root=pathlib.Path('/home/Nio/.local/share/nanafox-fx-production-backup');root.mkdir(mode=0o700,parents=True,exist_ok=True)
lock=(root/'run.lock').open('w')
try:fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
except BlockingIOError:
 print(json.dumps({'stage':'already_running'}),flush=True);raise SystemExit(0)
ssh=['ssh','-i',str(root/'id_ed25519'),'-o','IdentitiesOnly=yes','-o','BatchMode=yes','-o','StrictHostKeyChecking=yes','-o','UserKnownHostsFile='+str(root/'known_hosts'),'-o','ConnectTimeout=15','-o','ServerAliveInterval=30','-o','ServerAliveCountMax=6','nio@43.106.8.109','backup-v1']
...
try:
 with tempfile.TemporaryDirectory(prefix='run-',dir=root) as temp:
  ...
  prefix='backup/nanafox-postgres-backups/fx-production/'+stamp+'/'
  results=[]
