...
if command not in {'backup-v1','backup-studio-v1'}:
 sys.exit('Only production backup commands are permitted')
studio_only=command=='backup-studio-v1'
databases=['nanafox_studio_prod'] if studio_only else ['sub2api','nanafox_studio_prod']
root=pathlib.Path('/home/nio/backups/fx-production');r=pathlib.Path('/home/nio/recovery/live-production-20260906')
lock=(root/('studio-export.lock' if studio_only else 'export.lock')).open('w');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
...
with tempfile.TemporaryDirectory(prefix='run-',dir=root) as temp:
 ...
 if studio_only:
  args=args[:args.index('recovery/'+files[0])]+['recovery/studio.production.env','recovery/postgres.env','caddy','certificates/acme-v02.api.letsencrypt.org-directory/studio.nanafox.com']
 ...
 manifest={'timestamp_utc':stamp,'source':'studio-production' if studio_only else 'router-production','databases':databases,'files':records,'images':{x['Name'].lstrip('/'):x['Image'] for x in meta}}
