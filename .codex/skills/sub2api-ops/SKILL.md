---
name: sub2api-ops
description: "Sub2API deployment and operations on the current Router production stack, legacy-host safeguards, Docker, Caddy, NAS backups, and release targeting."
---
...
- Old host 108.160.133.141 has stopped Router/Studio production containers (restart=no) and legacy application state. Whole host is not decommissioned, but it is not a test deployment target. Do not deploy or restart legacy `sub2api-test` as a validation environment. pick/newapi are excluded from migration.
...
Before replacement, record target, candidate commit/image digest, current config/mount/network/volume/ports, backup status, validation and compatible rollback. The preflight is read-only and is not a deploy tool. `deploy-server.sh` is fully retired because there is no current test target and it cannot reproduce Router production. There is no verified automatic Router production replacement script in this repository: construct the reviewed candidate command from current inspect and the runbook, never guess old defaults.
