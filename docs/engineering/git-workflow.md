...
- `main`
  - 生产稳定分支
  - 应始终保持可发布状态
  - 不直接作为新版本测试部署入口；对应版本生产验证完成后，再从 release 分支合并进入 `main`

- `release/v0.1.xxx`
  - 版本发布分支，也是测试环境和生产候选部署分支
  - 每个上游版本同步或本仓库版本发布使用独立 release 分支，例如 `release/v0.1.116`
  - 测试环境先部署对应 release 分支验证；生产验证确认无问题后，再合并到 `main`
  - 不要跳过 release 分支，直接把未经生产验证的版本合入 `main` 后部署
