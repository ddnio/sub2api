...
| D. Auth identity schema core | `e9de839d` plus migrations `108_auth_identity_foundation_core.sql`, `109_auth_identity_compat_backfill.sql` | Current fork lacks `auth_identities`, `auth_identity_channels`, `pending_auth_sessions`, `identity_adoption_decisions`, `auth_identity_migration_reports`, and generated Ent code. Current `User` schema already has fork balance-notify/referral fields, so upstream `user.go` cannot be copied wholesale; it must be merged to add only `signup_source`, `last_login_at`, and `last_active_at`. Upstream migration filenames cannot be reused: current fork already has `108_add_account_stats_pricing.sql`, and upstream payment migrations `111`, `112`, and `117` already exist locally. | OPEN. Requires real DB preflight and a fork-specific migration design, likely new `128+` migration(s) that merge/crop upstream `108`/`109` instead of copying original filenames. |
