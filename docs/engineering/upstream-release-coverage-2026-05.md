...
| `0a4ece5f` | Proxy safety/intervals/SMTP/sort | PARTIAL | Websearch weighted provider sort is adapted so random weight factors stay paired with their provider during sorting, and channel websearch warning styling matches upstream intent. Account-stats intervals are already present in fork migration `108_add_account_stats_pricing.sql`, repository load/save code, and token-cost resolver; this checkpoint also preserves empty platform as wildcard for account-stats pricing rules while defaulting empty platform only for main channel `model_pricing`, with handler/service tests. SMTP timeout and notify/email-service cleanup remain under the balance/quota notification domain and are not closed by this websearch/account-stats slice. Verification: websearch manager tests, channel handler tests, account-stats targeted tests, frontend typecheck, and diff-check. |
...
| `7c729293` | Websearch quota + notify hint | PARTIAL | Websearch quota/reset subset adapted: backend provider config uses nullable `quota_limit`, legacy `0` remains accepted as unlimited for stored-data compatibility, manager exposes `ResetUsage`, admin route `POST /api/v1/admin/settings/web-search-emulation/reset-usage` resets provider usage, and Settings UI saves empty quota as `null`, shows `∞`, and offers reset for providers with usage. Notify email verification hint is not implemented here; it remains part of the balance/quota notification domain. Verification: targeted websearch/service tests, frontend typecheck, and diff-check. |
