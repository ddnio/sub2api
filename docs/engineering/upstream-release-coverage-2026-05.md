...
- Finish the first low-risk settings/export PR covering Subitems A-D through self-review, Kimi attempt, PR CI, and merge.
- Next after that PR merges: process Subitem E (`5f8e60a1` plus direct sort/search fixups) as a backend/frontend table sort/search PR. This is the largest PR #1538 behavior batch and must not be mixed with PR #1545.
- Then close Subitems F-I in one small remainder PR or audit-only closeout if dependency, dispatch, sidebar color, and applicable tests are already present.
