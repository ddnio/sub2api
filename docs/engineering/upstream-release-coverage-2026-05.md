...
| `8eb3f9e7` / PR #1785 | Auth identity foundation | Import audit completed; one independent OIDC security subset adapted; core implementation still open. | OPEN | Upstream PR #1785 has 279 changed files, about 79,981 insertions and 12,626 deletions, with 123 internal commits. A scratch `git cherry-pick -m 1 -x 8eb3f9e7` from current `origin/main` in `.claude/worktrees/audit-v0.1.115-pr1785` produced 81 unresolved conflicts, concentrated in Ent generated files/user schema, auth handlers/services, payment order/resume/provider snapshot paths, settings DTO/service, and frontend payment/profile/settings views. Therefore this PR must not be imported as one merge commit. Upstream internal commits `e01c1eac`, `721d7ab3`, `b6751f7e`, and `584ded21` are upstream `docs/superpowers/...` design/plan artifacts; they are used as audit input here and are not copied into the fork because this repo keeps execution plans under `docs/plans/` and durable engineering ledgers under `docs/engineering/`. Current fork already contains payment-b2 subsets from this PR family: `payment_orders.provider_instance_id`, `provider_key`, and `provider_snapshot` are in the Ent schema with migrations `112_add_payment_order_provider_key_snapshot.sql` and `117_add_payment_order_provider_snapshot.sql`; `/api/v1/payment/public/orders/resolve` and `PaymentResumeService` cover resume tokens; `payment_order_provider_snapshot.go` validates provider snapshot metadata; visible-method settings and load-balancer source routing exist; and frontend WeChat payment resume flow exists. This branch adapts only the independent security hunk from upstream internal commit `d3d42677`: OIDC callback now rejects mismatched `id_token.sub` and `userinfo.sub` when both are present, with focused `oidcSubjectsConsistent` unit coverage. The broader `d3d42677` PKCE/nonce enforcement, pending browser/session flow, synthetic-email-only identity behavior, profile binding, and remaining PR #1785 auth identity work are still tied to the missing auth identity schema/migrations plus pending OAuth/identity binding/profile/settings behavior. Process the remaining domains in upstream internal commit order, with real DB preflight before any auth identity migration is deployed. Do not mark this row closed until every internal domain is `MERGED`, `ADAPTED`, `PRESENT`, `REJECTED`, or `SKIP`, and release-level auth/payment/profile regression plus deployment checklist pass. |
...
| `755c7d50` | README revert to sponsor version | Upstream README revert only reconciles upstream sponsor/payment-doc churn. | SKIP | Because `78f691d2` sponsor churn is skipped and payment docs are handled separately, this README revert has no fork runtime behavior to port. |
...
| D. Auth identity schema core | `e9de839d` plus migrations `108_auth_identity_foundation_core.sql`, `109_auth_identity_compat_backfill.sql` | Adapted in the fork with new Ent schemas/generated code for `auth_identities`, `auth_identity_channels`, `pending_auth_sessions`, and `identity_adoption_decisions`; `User` keeps fork balance-notify/referral fields and adds only `signup_source`, `last_login_at`, and `last_active_at`. Fork-specific migration `128_auth_identity_foundation.sql` merges upstream `108` and applicable `109` behavior under a new filename because live environments already have fork migration `108_add_account_stats_pricing.sql` plus upstream-equivalent payment migrations `111`, `112`, and `117`. The migration creates `auth_identity_migration_reports`, backfills normal email identities, backfills recoverable linuxdo/wechat synthetic identities, and records OIDC/WeChat remediation reports where identity cannot be safely recovered automatically. | `ADAPTED`; verification: real DB read-only preflight across ToC test, ToC prod, and ToB prod; `go generate ./ent`; `go test -mod=mod -tags=unit ./ent/schema ./internal/repository -run 'TestAuthIdentityFoundationSchemas|TestAuthIdentityFoundationMigrationContainsExpectedBackfills'`. Runtime/schema deployment is still deferred until the whole `v0.1.115` gate is closed or a dedicated auth/schema deployment decision is made. |
| E. Pending OAuth and profile binding flows | `fbd0a2e3`, `c6d85924`, `6a75bd77`, `6ea3f42e`, `fb6204ea`, `7826e988`, `32059ae9`, `31d0183d`, `aaf4946b`, `bffcc204`, `4d10ba42`, `0fa47f18`, `85fc54b2`, `92041457`, `dcd5c43d`, `7e89bca5`, `f3986501`, `33b208ab`, `65efef1e`, `4c21320d`, `d4c0a991`, profile/frontend follow-ups | Subitem D schema is now present. The DB-backed `AuthPendingIdentityService` foundation and `AuthService.EntClient()` accessor are adapted from upstream so handlers can create browser-bound pending sessions and one-time completion codes. The pending OAuth exchange endpoint and helper functions are adapted from upstream `fbd0a2e3`, including browser-bound session cookies and suggested profile propagation. LinuxDo and OIDC invitation-required callbacks now also create DB-backed pending sessions while preserving the legacy `pending_oauth_token` fragment response for current frontend compatibility. Frontend auth API exposes `exchangePendingOAuthCompletion()` and `PendingOAuthExchangeResponse`; the callback views now try the browser-bound pending exchange first, then fall back to legacy fragment tokens when no DB-backed pending session is available. The backend generic upstream endpoints `/auth/oauth/pending/create-account` and `/auth/oauth/pending/bind-login` are now adapted from `6a75bd77`: create-account verifies a local email code, preserves this fork's existing registration gate/email suffix/reserved synthetic email/default balance/default subscriptions/invitation-code redemption/referral semantics, and binds the pending OAuth identity after account creation; if the submitted email already exists, the pending session moves to `bind_login_required` without consuming the session; bind-login validates the existing local password, enforces targeted-user ownership when present, binds the OAuth identity, records login activity, returns a token pair, and consumes the pending session. The implementation now fail-fast checks refresh-token cache and pre-existing OAuth identity ownership before local user creation to avoid half-created accounts on token/identity failures. This intentionally aligns upstream API shape while preserving existing online `redeem_codes` invitation data and `user_referrals` semantics instead of replacing them with upstream auth-source-default grants. Frontend API wrappers for create-account/bind-login are present; the legacy `pending_oauth_token` completion endpoints remain for compatibility. The frontend now adds a shared pending OAuth create-account form and wires LinuxDo/OIDC DB-backed `invitation_required` callbacks into create-account and bind-login completion without requiring a legacy fragment token; successful create/bind responses persist refresh-token context, load the access token, and redirect through the sanitized pending redirect. Upstream `c6d85924` was cherry-picked as `eced1922` with conflicts resolved against the fork's already-adapted LinuxDo/OIDC path: authenticated `/auth/oauth/linuxdo/bind/start` and `/auth/oauth/oidc/bind/start` route through the existing signed current-user bind cookie, profile UI displays avatar/source hints plus identity binding state, and frontend types/tests cover identity summaries. Upstream `92041457` is now cherry-picked as `40e85382` after restoring the upstream `auth_wechat_oauth.go` file instead of deleting it; this adds WeChat OAuth login/callback/payment/complete handlers, WeChat synthetic-email reservation, current fork routes for `/auth/oauth/wechat/start`, `/auth/oauth/wechat/callback`, `/auth/oauth/wechat/payment/*`, `/auth/oauth/wechat/complete-registration`, and authenticated `/auth/oauth/wechat/bind/start`, plus profile avatar adoption plumbing and profile UI tests. The `92041457` adaptation keeps provider default grants disabled here because `user_provider_default_grants`, dual WeChat OAuth mode settings, and auth-source-default UI/config are still Subitem F, not an implicit dependency to sneak into this slice. Current branch also adapts LinuxDo/OIDC pending bind 2FA from upstream `fb6204ea`/`7826e988` without importing auth-source default grants: bind-login returns `requires_2fa` and a temp token for TOTP-enabled target accounts; `/auth/login/2fa` verifies the TOTP code, reloads the browser-bound pending OAuth session, writes the `auth_identities` row, records login activity, consumes the pending session, clears OAuth pending cookies, and returns the normal auth response; LinuxDo/OIDC callback views render the TOTP challenge and complete with the existing 2FA endpoint. Current branch also adapts upstream `32059ae9`, `31d0183d`, and `aaf4946b`: successful password login and non-standard login completion now backfill email auth identities; repository email lookups normalize legacy case/spacing and fail closed on normalized duplicates; pending OAuth create-account existing-user detection uses the same normalized lookup semantics. Identity unbinding and remaining profile/activity follow-ups are still not closed. | OPEN. Continue in smaller backend/handler/frontend slices; current verification: `go test -mod=mod -tags=unit ./internal/service -run 'TestAuthPendingIdentityService|TestAuth|TestUser|TestAdminService'`, `go test -mod=mod -tags=unit ./internal/handler ./internal/server/routes ./internal/service -run 'TestApplySuggestedProfileToCompletionResponse|TestAuthPendingIdentityService|TestAuth|TestUser|TestAdminService'`, `go test -mod=mod -tags=unit ./internal/handler ./internal/server/routes ./internal/service -run 'Test(ApplySuggestedProfileToCompletionResponse|InvitationPendingPayloads|LinuxDoParseUserInfo|ParseOAuthProviderError|ParseLinuxDoTokenResponse|OIDCSyntheticEmailStableAndDistinct|OIDCSelectLoginEmailPrefersRealEmail|OIDCSubjectsConsistent|BuildOIDCAuthorizeURLIncludesNonceAndPKCE|AuthPendingIdentityService)'`, `GOCACHE=/Users/nio/project/nanafox/sub2api/.cache/go-build go test -tags=unit ./internal/service -run 'Test(AuthPendingIdentityService|AuthService_RegisterOAuthEmailAccount|AuthService_ValidatePasswordCredentials|AuthService_Register|AuthService_LoginOrRegisterOAuth)'`, `GOCACHE=/Users/nio/project/nanafox/sub2api/.cache/go-build go test -tags=unit ./internal/handler ./internal/server/routes -run 'Test(ApplySuggestedProfileToCompletionResponse|InvitationPendingPayloads|MergePendingCompletionResponse|EnsurePendingOAuthIdentityForUser|RejectPendingOAuthIdentityOwnedByAnotherUser|LinuxDoParseUserInfo|ParseOAuthProviderError|ParseLinuxDoTokenResponse|OIDCSyntheticEmailStableAndDistinct|OIDCSelectLoginEmailPrefersRealEmail|OIDCSubjectsConsistent|BuildOIDCAuthorizeURLIncludesNonceAndPKCE)'`, `pnpm --dir frontend exec vitest run src/views/auth/__tests__/oauthPendingCallback.spec.ts`, `pnpm --dir frontend run typecheck`, `GOCACHE="$PWD/../../.cache/go-build" go test -tags=unit ./internal/service ./internal/repository ./internal/handler ./internal/server/routes -run 'Test(AuthPendingIdentityService|ApplySuggestedProfileToCompletionResponse|InvitationPendingPayloads|MergePendingCompletionResponse|EnsurePendingOAuthIdentityForUser|RejectPendingOAuthIdentityOwnedByAnotherUser|LinuxDoParseUserInfo|ParseOAuthProviderError|ParseLinuxDoTokenResponse|OIDCSyntheticEmailStableAndDistinct|OIDCSelectLoginEmailPrefersRealEmail|OIDCSubjectsConsistent|BuildOIDCAuthorizeURLIncludesNonceAndPKCE|User|Auth)' -count=1`, `GOCACHE="$PWD/../../.cache/go-build" go test -tags=unit ./internal/service ./internal/handler ./internal/server/routes -run 'Test(AuthPendingIdentityService|CompletePendingOAuthBind|PendingOAuthBind|Login2FA|Totp|ApplySuggestedProfileToCompletionResponse|EnsurePendingOAuthIdentityForUser|RejectPendingOAuthIdentityOwnedByAnotherUser)' -count=1`, `GOCACHE="$PWD/../../.cache/go-build" go test -tags=unit ./internal/repository ./internal/service ./internal/handler -run 'Test(UserRepository.*Email|AuthService.*EmailIdentity|FindUserByNormalizedEmail|BindPendingOAuthLogin|Login2FACompletesPendingOAuthBindSession)' -count=1`, `GOCACHE="$PWD/../../.cache/go-build" go test -tags=unit ./internal/handler ./internal/server/routes ./internal/service ./internal/repository -run 'Test(LinuxDo|PendingOAuth|User|AuthPendingIdentity|Profile|Avatar|Notify|OAuthBind|OIDCSubjects|OIDCSelect|OIDCSynthetic|BuildOIDCAuthorize|Login2FA)' -count=1`, `pnpm --dir frontend exec vitest run src/views/auth/__tests__/oauthPendingCallback.spec.ts src/views/auth/__tests__/LinuxDoCallbackView.spec.ts src/views/auth/__tests__/OidcCallbackView.spec.ts`, `pnpm --dir frontend exec vitest run src/components/user/profile/__tests__/ProfileIdentityBindingsSection.spec.ts src/components/user/profile/__tests__/ProfileInfoCard.spec.ts src/views/auth/__tests__/LinuxDoCallbackView.spec.ts src/views/auth/__tests__/OidcCallbackView.spec.ts`, `pnpm --dir frontend run typecheck`, and `git diff --check`. A broader handler/service regex run still hits sandbox `httptest.NewServer` bind denial in unrelated OIDC/settings discovery tests. |
| F. Auth source defaults / WeChat OAuth modes | `4e0e6915`, `1521d503`, `e12599c1`, `2cebb0dc`, `54dc1767`, `40f7e832` plus migrations `110_pending_auth_and_provider_default_grants.sql` and `118_wechat_dual_mode_and_auth_source_defaults.sql` | Adds provider default-grant settings and dual WeChat OAuth mode settings. This is product/config behavior, not a pure migration. | OPEN. Needs product/config decision and test/prod settings preflight. |
