...
| `v0.1.114..v0.1.115` | Release PR ready | Current closeout branch `sync/v0.1.115-release-closeout` has mapped all 19 upstream first-parent entries to final outcomes. Merge, deploy test/prod, then bump `backend/cmd/server/VERSION` to `0.1.115` and create `fork/v0.1.115`. |
...
| G. Legacy identity migrations and reports | `113_normalize_legacy_wechat_provider_key.sql`, `114_auth_identity_migration_report_resolution.sql`, `115_auth_identity_legacy_external_backfill.sql`, `116_auth_identity_legacy_external_safety_reports.sql`, `7a9488ff`, `0a461d82`, `c624cce8`, `d08757ce`, `ebe75244`, `2626e8f2`, `7fbd5177` | Adapted with fork migration `115_auth_identity_legacy_external_backfill.sql` plus the report table created in `128_auth_identity_foundation.sql`; migration is guarded for absent `user_external_identities`, records WeChat openid-only remediation reports, and preserves live fork migration numbering. | `ADAPTED`; verification includes read-only live DB inventory and repository migration tests. |
...
Gate status: release PR ready. All 19 upstream first-parent entries in `v0.1.114..v0.1.115` have final local outcomes, and PR #1785/#1799 internal domains A-H are closed as `SKIP`, `PRESENT/ADAPTED`, or `ADAPTED`. Do not start `v0.1.115..v0.1.116` until this branch is merged to `main`, release-level CI/review passes, test/prod deployment is verified, `backend/cmd/server/VERSION` is bumped to `0.1.115`, and annotated tag `fork/v0.1.115` is created.
