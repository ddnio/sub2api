...
| E. Pending OAuth and profile binding flows | `fbd0a2e3`, `c6d85924`, `6a75bd77`, `6ea3f42e`, `fb6204ea`, `7826e988`, `bffcc204`, `4d10ba42`, `0fa47f18`, `85fc54b2`, `92041457`, `dcd5c43d`, `7e89bca5`, `f3986501`, `33b208ab`, `65efef1e`, `4c21320d`, `d4c0a991`, profile/frontend follow-ups | Subitem D schema is now present. The DB-backed `AuthPendingIdentityService` foundation and `AuthService.EntClient()` accessor are adapted from upstream so handlers can create browser-bound pending sessions and one-time completion codes. The pending OAuth exchange endpoint and helper functions are adapted from upstream `fbd0a2e3`, including browser-bound session cookies and suggested profile propagation. OAuth callbacks, profile binding APIs, frontend callback UI, email verification, 2FA bind flow, and identity unbinding are still not closed. | OPEN. Continue in smaller backend/handler/frontend slices; current verification: `go test -mod=mod -tags=unit ./internal/service -run 'TestAuthPendingIdentityService|TestAuth|TestUser|TestAdminService'` and `go test -mod=mod -tags=unit ./internal/handler ./internal/server/routes ./internal/service -run 'TestApplySuggestedProfileToCompletionResponse|TestAuthPendingIdentityService|TestAuth|TestUser|TestAdminService'`. |
