...
- Fork-renumbered upstream migrations require an explicit dependency-order audit before deployment. `v0.1.115` initially failed on ToC test because fork migration `128_auth_identity_foundation.sql` created tables needed by lower-numbered `115_auth_identity_legacy_external_backfill.sql`; the fix was a new immutable prerequisite migration `114a_auth_identity_legacy_backfill_prerequisites.sql`, not editing already published migration files.
- When running SSH commands that contain shell variables such as `$(date ...)` or `${TS}`, use single quotes around the remote command or escape `$`. Otherwise the local shell may expand the variable before SSH and produce ambiguous filenames such as `*_release_.sql`.
...
Gate status: final. PR #42 completed the earlier decision matrix; PR #43 bumped `backend/cmd/server/VERSION` from `0.1.112` to `0.1.113` and merged at `32787ca4`; annotated tag `fork/v0.1.113` was pushed for the merged fork commit. That tag is retained as an immutable historical marker and was not moved during the stricter recheck. PR #54 merged the stricter closeout at `6140ec11`, closing PR #1637 reprocessing and revalidating adapted PR #1655/#1666 surfaces. Release-level ToC test/prod and ToB production deployments from merged `main` are recorded below.
...
v0.1.113 release deployment record:

1. Merge and tag handling: PR #54 merged into `main` as `6140ec11` on 2026-05-04. Existing annotated tag `fork/v0.1.113` points at historical marker commit `32787ca4` and was intentionally not moved; this release was corrected forward on latest `main`.
2. CI and review: PR #54 passed GitHub `test`, `golangci-lint`, `backend-security`, and `frontend-security` checks on head `5fe4cd84`. Independent review found no merge blocker other than waiting for CI; CI passed before merge.
3. Real DB preflight: ToC test (`sub2api_test`), ToC prod (`sub2api`), and ToB prod (`sub2api_tob`) all returned no pre-existing `users.balance_notify_enabled`, `balance_notify_threshold`, `balance_notify_extra_emails`, `balance_notify_threshold_type`, or `total_recharged` columns, and no pre-existing `127_add_balance_notify_fields.sql` migration row. This made the additive `ADD COLUMN IF NOT EXISTS` path safe for all three environments.
4. Backups:
   - ToC test: `/home/nio/backups/sub2api_test_pre_v0_1_113_release_20260504-043459.sql` (`57M`).
   - ToC prod: `/home/nio/backups/sub2api_prod_pre_v0_1_113_release_20260504-043859.sql` (`999M`).
   - ToB prod: `/home/nio/backups/sub2api_tob_pre_v0_1_113_release_20260504-130251.sql` (`344M`).
5. ToC test deployment: deployed from `main` at `6140ec11` to `sub2api-test`; container `e81c189e54694952c7b8efc91ce55b9dfa682857062dc577e72e65f170f39502` started at `2026-05-04T04:38:06Z` and was healthy. Applied migrations include `107_add_account_cost_to_dashboard_tables.sql`, `108_add_account_stats_pricing.sql`, `125_add_channel_features_config.sql`, `126_migrate_websearch_emulation_to_tristate.sql`, and `127_add_balance_notify_fields.sql`. Post-migration `users` column shape matched the expected defaults/nullability. Local `/health` returned `{"status":"ok"}`, unauthenticated local `/v1/models` returned `401`, and the post-deploy severe-log scan was empty. Public `https://router-test.nanafox.com/health` returned `{"status":"ok"}` and public unauthenticated `/v1/models` returned `401`.
6. ToC production deployment: deployed from `main` at `6140ec11` to `sub2api-prod`; container `2dcfd3bd0c52c9cf6184416a55235ed892c49608dc3c43d05002d1e693d00626` started at `2026-05-04T04:39:28Z` and was healthy. The same migration set applied and the `users` column shape matched the expected defaults/nullability. Local `/health` returned `{"status":"ok"}`, unauthenticated local `/v1/models` returned `401`, public `https://router.nanafox.com/health` returned `{"status":"ok"}`, and public unauthenticated `/v1/models` returned `401`. One post-deploy log scan matching the broad `error` pattern found a live-traffic `WARN openai.forward_failed` with upstream HTTP/2 `INTERNAL_ERROR`; a narrower startup/migration severe scan for `panic|fatal|migration failed|preflight failed|postcheck failed|traceback|异常` was empty.
7. ToB production deployment: ToB uses the same backend/frontend runtime, so it was deployed instead of skipped. Deployed from `main` at `6140ec11` to `sub2api-prod` on `43.106.8.109`; container `bd271bcfee3acc12dee3839c4eb792a335336c903397fa3f670517e180e9598a` started at `2026-05-04T05:10:41Z` and was healthy. The same migration set applied and the `users` column shape matched the expected defaults/nullability. Local `/health` returned `{"status":"ok"}`, unauthenticated local `/v1/models` returned `401`, `https://fx.nanafox.com/health` returned `{"status":"ok"}`, public unauthenticated `/v1/models` returned `401`, and the post-deploy severe-log scan was empty.
8. Manual/UI follow-up: infrastructure deployment and API smoke are complete. Browser-level checks of the new admin settings websearch/notify screens, account quota notify controls, profile balance notify card, and account-stats pricing UI should be exercised with real admin/user sessions during the next product QA pass; the release gate is not blocked by this because automated frontend typecheck/Vitest plus live API/runtime smoke passed.
9. Rollback note: if a post-deploy regression appears, first preserve current logs, then redeploy the previous known-good `main` commit for the affected environment and restore the corresponding pre-release DB backup above only if the regression is schema/data related. Do not move `fork/v0.1.113`; fix forward on `main`.
...
2. `v0.1.114..v0.1.115` has now been closed under the stricter rule. Continue with `v0.1.115..v0.1.116` next.

#### PR #1637 internal commit coverage snapshot

This snapshot was refreshed while closing the `d402e722` PR #1637 reprocessing on branch `sync/v0.1.113-release-closeout`. It is a local release closeout snapshot: rows below are final local outcomes unless explicitly marked otherwise. Production-final status still requires independent review, PR CI, merge, tag, and deployment verification.
...
| `6c73b621` / PR #1734 | Payment Kyren Topup docs | Upstream docs target `docs/PAYMENT.md` and `docs/PAYMENT_CN.md`, which are not currently present in the fork. | ADAPTED | The fork keeps payment documentation in `docs/engineering/payment-b2-upstream-audit.md`, `payment-b2-deploy.md`, `payment-b2-deploy-log.md`, and `docs/ADMIN_PAYMENT_INTEGRATION_API.md`. Do not recreate upstream `docs/PAYMENT*.md` or add provider recommendation/referral content without a fork docs/product decision. |
| `51af8df3` / PR #1731 | Rate billing, payment config hardening, autofill avoidance, upstream response limit | Reprocessed by internal commits on branch `sync/v0.1.115-release-closeout`; upstream behavior is either present from payment-b2 or adapted with focused fork patches. | ADAPTED | `fd0c9a13`/`61a008f7` plaintext provider config plus legacy AES fallback are already present in `payment_config_providers.go`, `payment/crypto.go`, and payment-b2 docs. `235f7108` admin secret redaction is present through `decryptAndMaskConfig` and sensitive-field merge preservation. This branch adapts `44cdef79` by consuming subscription quota with `ActualCost`, `df57d277` by rejecting non-positive group/user-group multipliers while preserving fork account-rate `0` semantics, `bf0bbe0b` by raising non-stream upstream response read default to 128 MiB via `config.DefaultUpstreamResponseReadMaxBytes`, `948d8e6d` by adding password-manager ignore attributes to the API-key edit field, and `c3cb0280` by keeping fork's Alipay desktop `TradePreCreate` true-QR path while ensuring page-pay fallback returns only `pay_url` instead of fake QR. Verification: `go test -tags=unit ./internal/service -run 'TestCalculateCost_(ZeroMultiplierChargesZero|NegativeMultiplierChargesZero|RateMultiplier)|TestCalculateImageCost_(RateMultiplier|ZeroRateMultiplier)|TestAdminService_(CreateGroup_RejectsNonPositiveRateMultiplier|BatchSetGroupRateMultipliers)|TestBuildUsageBillingCommand_SubscriptionAppliesRateMultiplier|TestResolveUpstreamResponseReadLimit|TestReadUpstreamResponseBody'`, `go test -tags=unit ./internal/payment/provider -run 'TestCreateTradeUses(PagePayForDesktop|WapPayForMobile|PrecreateForDesktopWhenAvailable)'`, `pnpm --dir frontend exec vitest run src/components/payment/__tests__/paymentFlow.spec.ts src/components/payment/__tests__/PaymentStatusPanel.spec.ts`, and `pnpm --dir frontend run typecheck`. |
...
| `23def40b` | License MIT to LGPL v3.0 | Cherry-picked from upstream on branch `sync/v0.1.115-release-closeout`. | MERGED | Commit `ee34c9b6` updates `LICENSE` and README license references with `-x` source attribution. This is legal/project-policy, not runtime behavior; it does not require deployment. Human maintainers should still explicitly confirm that adopting upstream LGPL v3.0 is acceptable for the fork before the release gate is finalized. |
| `a8854947` / PR #1764 | Wxpay pubkey hardening and payment error i18n | Upstream PR #1764 contains only two internal commits: `79192cf6` Wxpay config validation and `40d4e167` payment error i18n. It does not include schema migrations or auth identity changes. | ADAPTED | Most behavior was already present from fork payment-b2: `NewWxpay` requires `appId`, `mchId`, `privateKey`, `apiV3Key`, `certSerial`, `publicKey`, and `publicKeyId`; validates 32-byte `apiV3Key`; parses PEM private/public keys eagerly; returns structured `WXPAY_CONFIG_*` errors; enabled provider create/update paths validate merged config at save time; payment order provider creation passes structured errors through; frontend payment/admin surfaces use `extractI18nErrorMessage`; `en.ts`/`zh.ts` include structured payment error messages; and wxpay `certSerial`/`publicKeyId` fields are non-optional in provider config. This branch adds missing release-local regressions for enabled Wxpay create/update validation and `apiError` i18n metadata localization. Verification: `go test -tags=unit ./internal/payment/provider -run 'TestNewWxpay|TestFormatPEM|TestMapWxState'`, `go test -tags=unit ./internal/service -run 'Test(CreateProviderInstanceValidatesEnabledWxpayConfig|UpdateProviderInstanceValidatesWxpayConfigWhenEnabling|UpdateProviderInstanceRejectsProtectedConfigChangesWhilePendingOrders|UpdateProviderInstanceAllowsSafeConfigChangesWhilePendingOrders)'`, `pnpm --dir frontend exec vitest run src/utils/__tests__/apiError.spec.ts`, `pnpm --dir frontend exec vitest run src/components/payment/__tests__/paymentFlow.spec.ts src/components/payment/__tests__/PaymentStatusPanel.spec.ts src/utils/__tests__/apiError.spec.ts`, and `pnpm --dir frontend run typecheck`. |
| `ffc9c387` / PR #1766 | Codex removed models and prompt-cache compatibility cleanup | Reprocessed as a Codex model policy adaptation on branch `sync/v0.1.115-release-closeout`. | ADAPTED | Public selectable/default model surfaces now remove upstream-retired `gpt-5`, `gpt-5.1*`, `gpt-5.2-codex`, and `gpt-5.4-nano` entries from OpenAI defaults, frontend whitelist/presets, and generated OpenCode config while preserving fork-added `gpt-5.5*`; the OpenCode-only `codex-mini-latest` config remains because upstream PR #1766 kept it. Backend compatibility remains: legacy request model strings normalize to current supported models (`gpt-5.4`, `gpt-5.3-codex`, or `gpt-5.3-codex-spark`) instead of being exposed publicly. Reviewer-found blockers were fixed before closeout: chat-completions prompt-cache injection now checks the original requested model as well as the upstream model so `gpt-4o`/Claude requests normalized to `gpt-5.4` do not get injected, and dynamic `PricingService` fallback no longer routes removed `gpt-5.1*`, `gpt-5.2-codex`, or `gpt-5.4-nano` variants to old removed-model prices. Verification: `go test -tags=unit ./internal/service -run 'TestNormalizeCodexModel|TestShouldAutoInjectPromptCacheKeyForCompat|TestShouldAutoInjectPromptCacheKeyForChatCompat|TestGetFallbackPricing|TestGetModelPricing|TestCalculateCost|TestCalculateCostWithServiceTier|TestShouldApplySessionLongContextPricing|TestResolveOpenAIForwardModel|TestNormalizeOpenAIModelForUpstream|TestOpenAIGatewayServiceRecordUsage|TestCodex|TestCursorWarmup'`, `go test -c -tags=unit ./internal/service`, `pnpm --dir frontend exec vitest run src/composables/__tests__/useModelWhitelist.spec.ts src/components/keys/__tests__/UseKeyModal.spec.ts`, `pnpm --dir frontend run typecheck`, and `git diff --check`. A broader `TestOpenAIGateway` run still includes httptest WebSocket cases that cannot bind ports in the current sandbox; targeted non-listener gateway usage tests passed. |
| `960b2bb8` | CLA workflow | Upstream CLA cannot be adopted verbatim by this fork. | REJECTED | The upstream workflow and `CLA.md` hard-code `Wei-Shaw/sub2api` document URLs and identify the upstream project owner. Applying it unchanged to the fork would create an incorrect legal/contributor workflow. If this fork wants CLA enforcement later, create a fork-specific CLA and workflow with the correct owner/repository. No runtime impact; no deployment. |
| `78f691d2` | Sponsor/readme update | Upstream sponsor/readme churn does not affect fork functionality. | SKIP | Current fork keeps its own sponsor list and has no `assets/partners/logos/bestproxy.png`. Non-functional branding churn is not required for upstream functional alignment. |
| `8eb3f9e7` / PR #1785 | Auth identity foundation | Replayed as a PR-level empty record after import audit, then adapted through smaller fork-safe commits. | ADAPTED | Direct `git cherry-pick -m 1 -x 8eb3f9e7` produced broad conflicts across Ent generated files, auth handlers/services, payment resume/provider snapshot paths, settings DTO/service, and frontend payment/profile/settings views. The fork keeps payment-b2 where already equivalent and adapts the auth identity foundation with fork migration `128_auth_identity_foundation.sql`, DB-backed pending OAuth sessions, profile identity binding/unbinding, auth-source default grants through settings, WeChat OAuth mode routing, legacy identity backfill/reporting, and user activity/profile support. Upstream design-only `docs/superpowers/...` artifacts are audit input only; fork durable decisions live in this ledger and `docs/plans/`. |
| `32107b4f` / PR #1795 | OpenAI image API sync | Image runtime/billing/scheduling feature family. Import audit completed after the pending-OAuth frontend slice. A scratch `git cherry-pick -m 1 -x 32107b4f` from `sync/v0.1.115-release-closeout` in `/private/tmp/sub2api-audit-pr1795-image` produced one conflict in `backend/internal/service/openai_account_scheduler.go`; the rest of the PR staged cleanly, adding `backend/internal/handler/openai_images.go`, `backend/internal/service/openai_images.go`, image route registration, endpoint normalization, image-capability account filtering, per-request/image billing-mode handling, and focused tests. The real import landed as `6417c4a7` with the scheduler conflict resolved by preserving fork scheduler behavior for quota-exceeded API key/Bedrock accounts, sticky-session clearing, transport compatibility, channel pricing, and billing logs while adding upstream's image capability filter. | ADAPTED | Verification: `go test -tags=unit ./internal/service -run 'TestOpenAIImages|Test.*Images|Test.*Image|TestModelPricingResolver|TestCalculateImageCost|TestOpenAIGatewayServiceRecordUsage_RecordsImageBillingMode|TestAccountSupportsOpenAIImageCapability|TestSelectAccountWithScheduler' -count=1`, `go test -tags=unit ./internal/handler -run 'TestOpenAIImages|Test.*Images|TestNormalizeInboundEndpoint|TestDeriveUpstreamEndpoint' -count=1`, `go test -tags=unit ./internal/server/routes -run 'Test.*Images|TestRegisterGatewayRoutes' -count=1`, `go test -c -tags=unit ./internal/service`, `go test -c -tags=unit ./internal/handler`, and `git diff --check`. A broader handler/routes/service regex also compiled and passed the targeted handler/routes tests, but the service package included existing `TestResolveAccountStatsCostFallsBackToModelFilePricing` expectation drift (`0.000125` expected vs current `gpt-5.1` -> `gpt-5.4` fallback `0.00025`); this pricing test predates the image import and must be fixed in the account-stats pricing lane, not hidden inside the image slice. |
| `4d0483f5` | GPT image model test feature | Image-adjacent test/admin feature. | ADAPTED | Upstream commit was cherry-picked as `b2b5b55a` after the PR #1795 image route/service foundation. The fork keeps its existing account-test service shape and frontend admin account-test modal while adding GPT image model test coverage/support. Follow-up commit `7148675a` aligns existing OpenAI account tests with the new optional image prompt parameter and closes the ledger row. Verification: `GOCACHE="$PWD/../../.cache/go-build" go test -tags=unit ./internal/service -run 'Test.*OpenAIAccount|Test.*Account.*Image|TestOpenAIImages|Test.*Image|TestSelectAccountWithScheduler|TestAccountSupportsOpenAIImageCapability' -count=1` from `backend`, `pnpm --dir frontend exec vitest run src/components/admin/account/__tests__/AccountTestModal.spec.ts src/composables/__tests__/useModelWhitelist.spec.ts`, and `git diff --check`. |
| `ddf80f5e` / PR #1799 | Auth identity foundation follow-up | Replayed as a PR-level empty record after import audit, then closed through the same fork-safe auth/profile/payment/settings adaptation sequence as PR #1785. | ADAPTED | Follow-up migration-order, payment resume, OAuth, WeChat, backend-mode, settings compatibility, profile identity, legacy backfill, and user-activity items are represented by local commits including `45065c23` adaptation, `40c8adbd`, `ca780981`, `6f09d6cb`, `aefbb5ce`, `9c7a38bf`, `99afc89f`, `d02b7338`, and related targeted tests. |
| `45065c23` | Auth migration-order test fix | Auth identity migration test ordering. | ADAPTED | Upstream fixes its `108` / `108a` / `109` integration-test order. The fork does not import those upstream migration filenames because live environments already use fork-specific numbering; applicable core/backfill behavior is merged into `128_auth_identity_foundation.sql` with fork-specific migration tests. Verification: `GOCACHE="$PWD/../../.cache/go-build" go test -tags=unit ./internal/repository -run 'TestAuthIdentityFoundationMigrationContainsExpectedBackfills|TestAuthIdentityFoundation' -count=1` from `backend`. |
| `c6d25f69` | Restore payment docs/files | Upstream restores payment docs and changes `.gitignore`/SettingsView links around those docs. | ADAPTED | Current fork already tracks docs through an adapted `.gitignore` that preserves `docs/engineering/` and `docs/plans/`; applying upstream's `docs/*` whitelist would hide fork engineering/planning docs. `docs/ADMIN_PAYMENT_INTEGRATION_API.md` already exists locally. `SettingsView.vue` has no current payment guide link block to update, and fork payment-b2 docs supersede upstream `docs/PAYMENT*.md`. No runtime change or deployment. |
| `1da4bd72` / PR #1802 | Profile auth bindings i18n | Cherry-pick produced no net diff after rerere because the current fork already had equivalent profile binding/i18n behavior from the auth identity closeout. | PRESENT | Recorded as an empty PR-level commit `b93550b3` with upstream `-x` attribution. |
| `755c7d50` | README payment-doc revert | Cherry-picked with conflict resolution limited to this commit's payment documentation wording. | ADAPTED | README files now describe payment as built in and mark external Sub2ApiPay as superseded, while preserving the fork's existing sponsor list and avoiding unrelated upstream sponsor churn. |

PR #1785 subitem matrix:

| Subitem | Upstream internal commits / files | Current fork read | Next action |
| --- | --- | --- | --- |
| A. Design docs | `e01c1eac`, `721d7ab3`, `b6751f7e`, `584ded21` | Upstream-only `docs/superpowers/...` design/plan artifacts are audit input. Fork durable docs live under `docs/plans/` and `docs/engineering/`. | `SKIP` as files; preserve decisions in this ledger. |
| B. OIDC subject consistency | Security hunk from `d3d42677` | Adapted by this branch without importing the pending-session/auth-identity parts of `d3d42677`. | `ADAPTED`; targeted handler unit passed. |
| C. Payment routing/resume/provider snapshot | `b51bc7ee`, `7c7924e9`, `e3f69e02`, `c0b24aef`, `9bebf1c1`, `561405ab`, `35aeeaa6`, `119f784d`, `276ce052`, `64e401e2`, `267844eb`, `0934f737`, `147ed42a`, and related tests | Fork payment-b2 already has provider instance routing, provider key/snapshot migrations, resume lookup/service, webhook provider resolution, visible method routing, and payment result tests. Upstream migrations `111_payment_routing_and_scheduler_flags.sql`, `112_add_payment_order_provider_key_snapshot.sql`, and `117_add_payment_order_provider_snapshot.sql` are byte-for-byte equivalent to current fork files. Read-only payment evidence review found no `OPEN` payment gaps: return URLs, fulfillment mismatch guard, webhook provider resolution, provider key/snapshot fields, resume token resolution, pinned read paths, wxpay snapshot validation, legacy fallback fail-closed behavior, merchant identity checks, and internal-result-page return URL restrictions are all present or adapted. | `PRESENT/ADAPTED`; keep as fork payment-b2, do not overwrite with upstream payment files. Rerun payment regression during release closeout. |
| D. Auth identity schema core | `e9de839d` plus migrations `108_auth_identity_foundation_core.sql`, `109_auth_identity_compat_backfill.sql` | Adapted in the fork with new Ent schemas/generated code for `auth_identities`, `auth_identity_channels`, `pending_auth_sessions`, and `identity_adoption_decisions`; `User` keeps fork balance-notify/referral fields and adds only `signup_source`, `last_login_at`, and `last_active_at`. Fork-specific migration `128_auth_identity_foundation.sql` merges upstream `108` and applicable `109` behavior under a new filename because live environments already have fork migration `108_add_account_stats_pricing.sql` plus upstream-equivalent payment migrations `111`, `112`, and `117`. Follow-up hotfix migration `114a_auth_identity_legacy_backfill_prerequisites.sql` creates the auth identity tables needed by lower-numbered `115_auth_identity_legacy_external_backfill.sql` before `115` runs. | `ADAPTED`; verification: real DB read-only preflight across ToC test, ToC prod, and ToB prod; `go generate ./ent`; `go test -mod=mod -tags=unit ./ent/schema ./internal/repository -run 'TestAuthIdentityFoundationSchemas|TestAuthIdentityFoundationMigrationContainsExpectedBackfills'`; post-deploy migration records for `114a`, `115`, and `128` verified in all three databases. |
| E. Pending OAuth and profile binding flows | `fbd0a2e3`, `c6d85924`, `6a75bd77`, `6ea3f42e`, `fb6204ea`, `7826e988`, `32059ae9`, `31d0183d`, `aaf4946b`, `bffcc204`, `4d10ba42`, `0fa47f18`, `85fc54b2`, `92041457`, `dcd5c43d`, `7e89bca5`, `f3986501`, `33b208ab`, `65efef1e`, `4c21320d`, `d4c0a991`, profile/frontend follow-ups | DB-backed pending OAuth, account completion/bind-login, LinuxDo/OIDC/WeChat pending flows, pending bind 2FA, profile identity summaries, avatar/profile hydration, email identity sync, normalized email lookup, and identity unbinding are adapted while preserving fork invitation/referral semantics. | `ADAPTED`; verification covered by targeted backend auth/handler/repository/service tests, frontend OAuth callback/profile tests, frontend typecheck, and `git diff --check`. |
| F. Auth source defaults / WeChat OAuth modes | `4e0e6915`, `1521d503`, `e12599c1`, `2cebb0dc`, `54dc1767`, `40f7e832` plus migrations `110_pending_auth_and_provider_default_grants.sql` and `118_wechat_dual_mode_and_auth_source_defaults.sql` | Upstream provider default-grant behavior is adapted through fork settings keys instead of importing a separate `user_provider_default_grants` table; WeChat OAuth supports open/mp mode selection and preserves fork registration, invitation, and referral data. | `ADAPTED`; verification includes `setting_handler_auth_source_defaults_test.go`, auth-source signup/first-bind tests, WeChat OAuth mode tests, and public settings/api-contract coverage. |
| G. Legacy identity migrations and reports | `113_normalize_legacy_wechat_provider_key.sql`, `114_auth_identity_migration_report_resolution.sql`, `115_auth_identity_legacy_external_backfill.sql`, `116_auth_identity_legacy_external_safety_reports.sql`, `7a9488ff`, `0a461d82`, `c624cce8`, `d08757ce`, `ebe75244`, `2626e8f2`, `7fbd5177` | Adapted with fork migration `115_auth_identity_legacy_external_backfill.sql` plus prerequisite migration `114a_auth_identity_legacy_backfill_prerequisites.sql` and foundation migration `128_auth_identity_foundation.sql`; migration is guarded for absent `user_external_identities`, records WeChat openid-only remediation reports, and preserves live fork migration numbering. | `ADAPTED`; verification includes read-only live DB inventory, repository migration tests, and release deployment verification across ToC test, ToC production, and ToB production. |
| H. User activity/profile support | `13d9780d`, `bf3ef2d1`, `ed01c599`, `7309c02f`, `17c6348b`, `6d51834a`, avatar/profile commits | `last_login_at` and `last_active_at` are in the fork schema/migration, admin user list DTO/API/frontend support them, current-user/profile handlers update activity, and profile/avatar UI support is adapted. | `ADAPTED`; verification includes admin activity handler tests, DTO mapper tests, repository sort tests, user service/profile tests, and frontend admin/profile tests. |
...
- Implementation implication: the fork migration is new `128_auth_identity_foundation.sql`, not upstream filenames. It creates the auth identity core, backfills normal email identities, backfills recoverable linuxdo/wechat synthetic identities, and records manual recovery reports for synthetic OIDC/WeChat cases that cannot be safely resolved automatically. The follow-up fork migration `115_auth_identity_legacy_external_backfill.sql` is guarded for environments without `user_external_identities`, so it is safe for the observed live databases while still covering any legacy external-identity table if present elsewhere.
...
Gate status: final. All 19 upstream first-parent entries in `v0.1.114..v0.1.115` have final local outcomes, PR #1785/#1799 internal domains A-H are closed as `SKIP`, `PRESENT/ADAPTED`, or `ADAPTED`, and no release-local unresolved `HOLD`, `REOPENED`, `PORT`, or `PARTIAL` entries remain. The release gate merged to `main` as `ce9d6e3f`; migration-order hotfix `61bb3cee` added immutable prerequisite migration `114a_auth_identity_legacy_backfill_prerequisites.sql`; marker commit `48d4a8de` bumped `backend/cmd/server/VERSION` to `0.1.115`; annotated tag `fork/v0.1.115` points at `48d4a8de`.

Post-final gap fix: branch `fix/v0.1.115-settings-ordered` forward-fixes missed v0.1.115 settings/WeChat/profile capability gaps discovered after the initial `fork/v0.1.115` marker. The branch keeps the existing marker/tag immutable and does not rewrite deployed migrations. Upstream-attributed cherry-picks in this patch set cover `ee3f158f`, `2cebb0dc`, `54dc1767`, `40f7e832`, `067eb23d`, `4f6966d7`, and `cd0338fb`; local conflict adaptations then remove duplicate frontend helper declarations, restore `118_wechat_dual_mode_and_auth_source_defaults.sql` to the upstream tag-safe checksum/content, allow backend WeChat OAuth requests for any explicitly enabled open/mp capability instead of only the stored preferred mode, complete the missing Auth Source Defaults admin UI, and adjust tests for the default-off OpenAI experimental scheduler setting. Migration note: deployed v0.1.115 environments previously recorded `114a`, `115`, and `128` but not `118`; applying the current `118` file after `119..128` is safe because it inserts missing setting rows with `ON CONFLICT DO NOTHING` and does not overwrite existing auth-source grant settings. If any ad-hoc environment already applied the earlier 32-line branch-local `118`, it must be handled as a checksum exception or forward migration before startup.

Post-final local verification: `cd backend && GOCACHE="$PWD/../../.cache/go-build" go test -tags=unit ./internal/handler ./internal/handler/admin ./internal/service ./internal/repository -run 'TestWeChat|TestSettingHandler_GetPublicSettings_ExposesWeChatOAuthModeCapabilities|TestSettingService_GetPublicSettings_ExposesWeChatOAuthModeCapabilities|TestSettingHandler_UpdateSettings_PersistsPaymentVisibleMethodsAndAdvancedScheduler|TestSettingService_GetWeChatConnectOAuthConfig_UsesDatabaseOverrides|TestOpenAIGatewayService_OpenAIAccountScheduler|TestDefaultOpenAIAccountScheduler|Test.*Migration|TestAuthIdentityFoundation|TestAuthIdentityLegacy'`; `pnpm --dir frontend exec vitest run src/components/auth/__tests__/WechatOAuthSection.spec.ts src/components/user/profile/__tests__/ProfileIdentityBindingsSection.spec.ts src/views/auth/__tests__/WechatCallbackView.spec.ts src/views/admin/__tests__/SettingsView.spec.ts src/api/__tests__/settings.authSourceDefaults.spec.ts src/api/__tests__/settings.wechatConnect.spec.ts`; `pnpm --dir frontend run typecheck`; `git diff --check`. Independent review found the backend dual open/mp mode bug and the migration checksum precondition; both are recorded above, with the dual-mode backend bug fixed in this patch set. A second independent review before deployment found a WeChat pending-session blocker: `complete-registration` accepted non-registration pending sessions and did not persist canonical identity/channel metadata. Commit `7ec75cb3` fixed it by requiring WeChat `login` sessions whose completion response is `invitation_required`, normalizing OAuth identity metadata to include `display_name`/`avatar_url`, and reusing the existing WeChat runtime identity binding repair to create channel records.

Post-final guard verification after `7ec75cb3`: `cd backend && GOCACHE="$PWD/../.cache/go-build" go test -count=1 -tags=unit ./internal/handler -run 'TestCompleteWeChatOAuthRegistrationAfterInvitationPendingSession|TestCompleteWeChatOAuthRegistrationRejectsAdoptExistingUserSession|Test.*WeChat|Test.*Setting|TestUserHandler.*Profile|TestUserHandler.*Identity|TestUserHandlerUnbind'` passed; `cd backend && GOCACHE="$PWD/../.cache/go-build" go test -count=1 -tags=unit ./internal/service -run 'TestAuthPendingIdentity|TestSettingService|TestUserService|TestAuthService_LoginOrRegisterOAuthWithTokenPair'` passed; `cd backend && GOCACHE="$PWD/../.cache/go-build" go test -count=1 -tags=unit ./cmd/server` passed; `pnpm --dir frontend exec vitest run src/components/auth/__tests__/WechatOAuthSection.spec.ts src/components/user/profile/__tests__/ProfileIdentityBindingsSection.spec.ts src/components/user/profile/__tests__/ProfileInfoCard.spec.ts src/views/auth/__tests__/WechatCallbackView.spec.ts src/views/admin/__tests__/SettingsView.spec.ts src/views/user/__tests__/ProfileView.spec.ts src/api/__tests__/settings.authSourceDefaults.spec.ts src/api/__tests__/settings.wechatConnect.spec.ts src/api/__tests__/user-oauth-binding.spec.ts` passed with 42 tests; `git diff --check` passed. A broader `go test -count=1 -tags=unit ./internal/service ./cmd/server` compiled `cmd/server` but failed in the existing service-package wide suite after 84s; the failure output was from pre-existing gateway/Google OAuth/worker-pool test behavior and was not used as release evidence for this WeChat/settings guard.

Release-level deployment evidence:

1. Initial ToC test deployment from `ce9d6e3f` exposed a real migration-order blocker: `115_auth_identity_legacy_external_backfill.sql` failed because `auth_identity_migration_reports` did not exist. Production was not deployed on this failed build. The test container was stopped, root cause was traced to fork-specific migration renumbering, and hotfix `61bb3cee` added `114a_auth_identity_legacy_backfill_prerequisites.sql`.
2. Local verification for the hotfix: `cd backend && GOCACHE=/Users/nio/project/nanafox/sub2api/.cache/go-build go test -tags=unit ./internal/repository` passed; `git diff --check` passed. Self-review confirmed `114a` sorts before `115`, creates only prerequisite tables/indexes, performs no data backfill, and does not modify existing migration files.
3. ToC test deployment: deployed from `main` at `48d4a8de` to `sub2api-test` on `108.160.133.141`; repo `backend/cmd/server/VERSION` is `0.1.115`; local `/health` returned `{"status":"ok"}`, local unauthenticated `/v1/models` returned `401`, container was healthy on `127.0.0.1:8081`, post-deploy severe-log scan was empty, public `https://router-test.nanafox.com/health` returned `{"status":"ok"}`, and public unauthenticated `/v1/models` returned `401`.
4. ToC production deployment: deployed from `main` at `48d4a8de` to `sub2api-prod` on `108.160.133.141`; repo `backend/cmd/server/VERSION` is `0.1.115`; local `/health` returned `{"status":"ok"}`, local unauthenticated `/v1/models` returned `401`, container was healthy on `127.0.0.1:8080`, post-deploy severe-log scan was empty, public `https://router.nanafox.com/health` returned `{"status":"ok"}`, and public unauthenticated `/v1/models` returned `401`.
5. ToB production deployment: deployed from `main` at `48d4a8de` to `sub2api-prod` on `43.106.8.109`; repo `backend/cmd/server/VERSION` is `0.1.115`; local `/health` returned `{"status":"ok"}`, local unauthenticated `/v1/models` returned `401`, container was healthy on `127.0.0.1:8080`, post-deploy severe-log scan was empty, public `https://fx.nanafox.com/health` returned `{"status":"ok"}`, and public unauthenticated `/v1/models` returned `401`.
6. Post-deploy migration record check: ToC test `sub2api_test`, ToC prod `sub2api`, and ToB prod `sub2api_tob` all recorded `114a_auth_identity_legacy_backfill_prerequisites.sql`, `115_auth_identity_legacy_external_backfill.sql`, and `128_auth_identity_foundation.sql` in `schema_migrations`.
7. Backup note: the required pre-deploy backup step was missed before the initial `v0.1.115` deployment attempt. Current post-deploy backups were created immediately after successful verification for rollback/reference: ToC test `/home/nio/backups/sub2api_test_post_v0_1_115_release_20260505-062037.sql` (`56M`), ToC prod `/home/nio/backups/sub2api_prod_post_v0_1_115_release_20260505-062037.sql` (`1.1G`), and ToB prod `/home/nio/backups/sub2api_tob_post_v0_1_115_release_20260505-142038.sql` (`346M`). Future migration releases must run and record pre-deploy backups before deployment.
8. Post-final settings/OAuth guard deployment: deployed `main` at `7ec75cb3` after pre-deploy backups. ToC test backup `/home/nio/backups/sub2api_test_pre_v0115_wechat_pending_guard_20260505-122729.sql` (`56M`, mode `600`); deployed to `sub2api-test` on `108.160.133.141`, container `70d9cea0ef0b` healthy, local `/health` and public `https://router-test.nanafox.com/health` returned `{"status":"ok"}`, local and public unauthenticated `/v1/models` returned `401`, `schema_migrations` recorded `118_wechat_dual_mode_and_auth_source_defaults.sql`, and the severe-log scan for `panic|fatal|migration failed|preflight failed|postcheck failed|traceback|异常` was empty.
9. ToC production backup `/home/nio/backups/sub2api_prod_pre_v0115_wechat_pending_guard_20260505-123008.sql` (`1.1G`, mode `600`); deployed `7ec75cb3` to `sub2api-prod` on `108.160.133.141`, container `1a5649d7a235` healthy, local `/health` and public `https://router.nanafox.com/health` returned `{"status":"ok"}`, local and public unauthenticated `/v1/models` returned `401`, migration `118_wechat_dual_mode_and_auth_source_defaults.sql` was recorded, and the severe-log scan was empty.
10. ToB production backup `/home/nio/backups/sub2api_tob_pre_v0115_wechat_pending_guard_20260505-203128.sql` (`346M`, mode `600`); deployed `7ec75cb3` to `sub2api-prod` on `43.106.8.109`, container `0a81aa340f25` healthy, local `/health` and public `https://fx.nanafox.com/health` returned `{"status":"ok"}`, local and public unauthenticated `/v1/models` returned `401`, migration `118_wechat_dual_mode_and_auth_source_defaults.sql` was recorded, and the severe-log scan was empty.
...
9. `v0.1.124` is committed on `release/v0.1.124`, pushed to `origin`, backed up, deployed to shared test at `b4e29f33`, and verified with generic smoke checks plus the `136_migrate_referrals_to_affiliates.sql` migration relationship check. Production and FX deployment are pending shared-test manual validation.
