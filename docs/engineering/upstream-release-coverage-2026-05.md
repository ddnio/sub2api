...
| `ffc9c387` / PR #1766 | Codex removed models and prompt-cache compatibility cleanup | Current fork still intentionally exposes or maps legacy Codex-compatible model IDs. | OPEN | Focused audit found the fork still has `gpt-5.1-codex*`, `gpt-5.2-codex`, `gpt-5.4-nano`, and `codex-mini-latest` paths in OpenAI constants, Codex transform, billing fallback, key-use config, and model whitelist. Do not blindly delete them. This needs a dedicated Codex model policy decision and regression tests before closing. |
