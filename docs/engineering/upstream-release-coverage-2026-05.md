...
| `32107b4f` / PR #1795 | OpenAI image API sync | Image runtime/billing/scheduling feature family. Import audit completed after the pending-OAuth frontend slice. A scratch `git cherry-pick -m 1 -x 32107b4f` from `sync/v0.1.115-release-closeout` in `/private/tmp/sub2api-audit-pr1795-image` produced one conflict in `backend/internal/service/openai_account_scheduler.go`; the rest of the PR staged cleanly, adding `backend/internal/handler/openai_images.go`, `backend/internal/service/openai_images.go`, image route registration, endpoint normalization, image-capability account filtering, per-request/image billing-mode handling, and focused tests. | OPEN | This is suitable as a dedicated v0.1.115 image slice, not a wholesale replacement of existing fork routing/billing. Conflict resolution must preserve fork scheduler behavior for quota-exceeded API key/Bedrock accounts, sticky-session clearing, transport compatibility, channel pricing, and billing logs while adding upstream's image capability filter. Required verification: image handler/service tests from PR #1795, scheduler selection tests, model pricing resolver tests for `BillingModeImage`/per-request channel pricing, gateway endpoint normalization tests, existing OpenAI gateway usage/billing tests, and payment/billing regression if request billing paths change. |
| `4d0483f5` | GPT image model test feature | Image-adjacent test/admin feature. | OPEN | Process immediately after PR #1795 in the same image slice; do not mark closed until the image API route/service and associated tests are present. |
