...
- After all items inside a release appear closed, run a release-level closeout review before updating the fork release marker/tag or starting the next release. The closeout must re-run the upstream first-parent list, confirm every commit/PR has a final outcome with evidence, confirm runtime tests/CI/deploy records for changed code, and check that the ledger has no unresolved `HOLD`, `REOPENED`, `PORT`, or `PARTIAL` entries for that release.
...
4. Confirm local tests, GitHub CI, Kimi review, deployment notes, and log checks exist for every runtime PR in the release.
5. Only after this closeout review passes, update the fork release marker and create/push the `fork/vX.Y.Z` tag.
...
| `16126a2c` / PR #1545 | Smooth sidebar collapse | Upstream merge commit is not an ancestor and conflicts with fork sidebar, but the earlier blanket rejection is too broad under the reset rule. | REOPENED | Re-audit current fork `AppSidebar.vue` and `style.css`. If the behavior is still useful, port/adapt only the compatible collapse behavior and tests. If rejected, document the exact fork UX reason and evidence. |
...
Gate status: reopened. PR #39 bumped the fork release marker from `0.1.110` to `0.1.111` and merged at `d2a3e5a9`; annotated tag `fork/v0.1.111` points at that merged fork commit. That tag is retained as a historical marker, but the release gate is not final until PR #1538 and PR #1545 are reprocessed under the stricter rule.
...
- Close it through the PR branch's internal commits in order, using the planned subitems below. Functional regrouping is allowed only for direct fixup commits that repair the same behavior.
...
5. Run the targeted tests for the touched group. Do not combine multiple upstream internal commits into one PR unless the later commit is a direct fixup for the earlier one.
6. After merge and CI, deploy test/prod only if runtime behavior changed; docs-only audit PRs do not deploy.
...
| --- | --- | --- | --- | --- | --- |
| A. Table scrollbar UI | `fe211fc5`: `frontend/src/components/common/DataTable.vue`, `frontend/src/style.css` | Fork already has DataTable changes from later frontend work; scrollbar CSS needs visual/code audit before import because upstream uses broad style overrides. | Try the internal commit first. If it conflicts or introduces heavy global CSS, adapt only the scrollbar visibility fix or reject with UX reason. | Targeted DataTable/component tests if behavior changes; `pnpm --dir frontend typecheck`; browser visual check if CSS changes. | `PRESENT`, `ADAPTED`, or `REJECTED` with exact CSS/view evidence. |
| B. Account status filter | `d8fa38d5`: `frontend/src/components/admin/account/AccountTableFilters.vue` and its test | Current fork account filters may have diverged. | Try the internal commit. If fork UI already handles status correctly, close as `PRESENT`; otherwise port the minimal filter fix and keep current fork layout. | Targeted account filter vitest; frontend typecheck if changed. | Status filter behavior is proven or fixed. |
| C. Table page-size settings | `ad80606a`, `13124059`, `67a05dfc`, `f480e573` table-default portions: settings service/handler, API contract, app config, admin settings UI, table preference utils/tests. | Mostly present in fork, but defaults are inconsistent: public contract/tests use `[10,20,50]` while `InitializeDefaultSettings` still has `[10,20,50,100]`. | Group these because later internal commits are direct fixups to the same feature. Try/apply only the relevant table-default hunks; keep fork UI/API if already stronger. Normalize the default mismatch or explicitly reject upstream's `[10,20,50,100]` with reason. Record the max page-size policy because Subitem E's backend pagination limit depends on it. | Backend settings tests and API contract tests; frontend table preference and app store tests; `pnpm --dir frontend typecheck` if frontend files change. | `PRESENT` if only evidence is needed; otherwise `ADAPTED` after the default mismatch, max-page-size decision, and tests are fixed. |
| D. Export filters | `66e15a54`: account/proxy/redeem export handlers, tests, redeem API client | Fork export behavior is unproven. | Try the internal commit. If backend/frontend params conflict, adapt only export filter propagation. | Targeted admin export handler tests; frontend typecheck if API client changes. | Export uses current filters or is explicitly rejected with reason. |
| E. Backend/frontend table sort and search | `5f8e60a1` plus direct fixups `62962c05`, `b6946e78`, `269c7a06`: pagination primitives, repository sort/search, handlers, admin/frontend query wiring, tests. | This is the largest behavior change and must not be one uncontrolled PR. | First try internal commit in scratch to identify conflicts, then split by resource family: announcements, usage, API keys, accounts, groups, channels, promo, proxies, redeem, users. Apply fixup commits only to the resource they repair. | Matching repository/handler tests for each family; targeted vitest/typecheck for touched views. | Every resource family is `MERGED`, `ADAPTED`, `PRESENT`, or `REJECTED` before E closes. |
| F. Frontend dependency security | `b6bc0423`: `frontend/package.json`, `frontend/pnpm-lock.yaml` axios upgrade | Current fork dependency state must be checked separately. | Do not bundle with UI/table behavior. Compare current axios version/lock against the upstream security bump. If already covered, close as `PRESENT`; otherwise make a dependency-only PR with lockfile and frontend install/test. | `pnpm --dir frontend install --frozen-lockfile`, frontend security/audit-equivalent CI, typecheck/build if dependency changes. | Axios security bump is present or dependency PR merged. |
| G. Messages dispatch hydration preservation | `7dc7ff22`: API key repository hydration tests/helpers | Fork messages dispatch was previously ported. | Audit against fork dispatch tests. If missing, port only preservation tests/fix; do not mix with table sort. | Targeted API key repository tests. | Dispatch mapping preservation is proven or fixed. |
| H. Sidebar SVG color follow-up | `f480e573` sidebar-color portions only | PR #1545 owns smooth sidebar collapse; current fork sidebar has local payment/contact/navigation changes. | Do not import broad sidebar changes under PR #1538. Only audit the SVG color preservation hunk; defer collapse behavior to PR #1545. | AppSidebar test/typecheck if changed. | Color hunk is present/adapted/rejected; collapse remains PR #1545. |
| I. Applicable upstream tests | All PR #1538 added tests. | Some tests conflict because fork behavior diverged. | Port tests with the internal commit or resource family they cover, not as a separate large test dump. Replace incompatible upstream assertions with fork-equivalent assertions and record the reason. | Same as owning subitem. | All applicable upstream tests are either ported, adapted, or rejected with reason. |

Immediate next runtime PR after this docs/audit PR:

- Work only on Subitem A (`fe211fc5` table scrollbar UI), because it is the first upstream internal commit in PR #1538.
- Do not start table page-size settings, backend pagination, sort/search, dependency, or PR #1545 work until the preceding PR #1538 internal-commit subitems are closed in order, except when a later internal commit is a direct fixup for the same subitem.
