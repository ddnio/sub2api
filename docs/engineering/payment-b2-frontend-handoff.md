...
- 后端 `/payment/limits` / `/payment/checkout-info` 已叠加全局最低/最高充值金额，避免 Provider 单笔最低额低于系统最低额时前端展示可下单但后端拒单。
...
2. 创建/编辑订阅套餐。注意 payment v2 读取 `subscription_plans`；旧 `payment_plans` 会由 `120b` migration 按 `group_id + name` 幂等补齐，`123` 会把绑定普通分组、停用分组或缺失分组的在售套餐下架。
3. 普通用户进入 `/purchase` 下单，确认侧边栏有“我的订单”入口 `/orders`。
4. 微信扫码支付。
