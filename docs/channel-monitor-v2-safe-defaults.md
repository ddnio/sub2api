...
1. **H2** — Migration 212 and code defaults set `channel_monitor_mode=v2`, so every upgrade without an existing key auto-opts into passive V2, stops V1 probes, and may leave users on an empty V2 view.
...
- Migration `212_channel_monitor_mode.sql`: insert `'v1'` when key missing.
- If 212 already applied with old checksum, add migration checksum compatibility (do not force rewrite applied DBs to v1).
