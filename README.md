...
When `security.url_allowlist.enabled=false`, the system performs minimal URL validation and **allows HTTP URLs by default** (dev-friendly mode). Docker Compose templates in this fork keep a stricter HTTPS-only fallback unless you explicitly override the environment variable. For production, keep this tightened:
