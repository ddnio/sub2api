...
describe('usageLoadQueue', () => {
  ...
  // Non-Anthropic platforms: execute directly without queueing.

  it('does not throttle Gemini requests', async () => {
    const timestamps: number[] = []
    const makeFn = () => async () => {
      timestamps.push(Date.now())
      return 'ok'
  ...
  it('does not throttle OpenAI requests', async () => {
    const timestamps: number[] = []
    const makeFn = () => async () => {
      timestamps.push(Date.now())
      return 'ok'
  ...
  // Anthropic apikey accounts are not throttled.

  it('does not throttle Anthropic apikey accounts', async () => {
    const timestamps: number[] = []
    const makeFn = () => async () => {
      timestamps.push(Date.now())
      return 'ok'
  ...
  it('passes Anthropic return values through', async () => {
  ...
  it('passes non-Anthropic return values through', async () => {
