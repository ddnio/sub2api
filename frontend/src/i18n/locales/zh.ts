export default {
  ...
  docs: {
    ...
    quickStart: {
      title: '快速开始',
      subtitle: '选择你使用的客户端，按对应指南配置 API 地址和密钥。',
      baseUrlLabel: '基础地址',
      getKeyStep: '创建 API Key',
      getKeyDesc: '登录控制台，在 API Key 页面创建或复制你的密钥。',
      chooseToolStep: '选择客户端',
      chooseToolDesc: '根据你的工具选择配置指南，复制示例后替换为自己的 API Key。',
      toolCards: {
        claudeCode: '配置 Claude Code 连接本站上游。',
        codexCli: '配置 Codex CLI 使用 Responses API 路由。',
        opencode: '配置 OpenCode provider 和模型参数。',
        apiUsage: '查看 OpenAI 兼容 API 的基础调用方式。'
      }
    }
  },
