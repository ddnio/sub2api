...
describe('OAuth captcha start API', () => {
  ...
  it('adds the stored affiliate code to captcha-gated POST starts', async () => {
    window.sessionStorage.setItem('oauth_aff_code', 'AFF-STORED')
    post.mockResolvedValue({ data: { authorize_url: 'https://provider.example/authorize' } })

    await startOAuthLogin(
      { provider: 'oidc', params: { redirect: '/dashboard' } },
      { turnstile_token: 'proof' }
    )

    expect(post).toHaveBeenCalledWith('/auth/oauth/oidc/start', { turnstile_token: 'proof' }, {
      params: { redirect: '/dashboard', aff_code: 'AFF-STORED' }
    })
  })
})
