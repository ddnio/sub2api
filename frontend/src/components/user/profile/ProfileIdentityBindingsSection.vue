...
function getBindingStatusForUser(user: User | null | undefined, provider: UserAuthProvider): boolean {
  if (provider === 'email') {
    ...
    const nested = user?.identities?.email ?? user?.auth_bindings?.email ?? user?.identity_bindings?.email
  ...
  const nested = user?.identities?.[provider] ?? user?.auth_bindings?.[provider] ?? user?.identity_bindings?.[provider]
  const normalized = normalizeBindingStatus(nested)
  return normalized ?? false
}
...
function getBindingDetails(provider: UserAuthProvider): UserAuthBindingStatus | null {
  const binding =
    currentUser.value?.identities?.[provider] ??
