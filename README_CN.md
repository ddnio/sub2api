...
当 `security.url_allowlist.enabled=false` 时，系统仅执行最小 URL 校验，且**默认允许 HTTP URL**（开发友好模式）。本 fork 的 Docker Compose 模板会保持更严格的仅 HTTPS fallback，除非你显式覆盖环境变量。生产环境建议保持这个收紧配置：
